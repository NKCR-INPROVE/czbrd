    ...
    enum Actions {
        BYQUERY {
                    ...
                    void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
                        ...
                        try {
                            ...
                            if (req.getParameterValues("facet") != null) {
                                for (String facet : req.getParameterValues("facet")) {
                                    query.addFacetField(facet);
                                }
                            }
                            
                            if (req.getParameterValues("facet.range") != null) {
                                for (String facet : req.getParameterValues("facet.range")) {
                                    query.addNumericRangeFacet(facet, 0, 14, 1);
                                }
                            }
                            
                            query.add("f.mer_KBLOKPH.facet.sort", "index");
        ...
        ASIS {
                    ...
                    void doPerform(HttpServletRequest req, HttpServletResponse resp) throws Exception {
                        ...
                        PrintWriter out = resp.getWriter();
                        try {
                            JSONObject json = new JSONObject(IndexQuery.json(req.getQueryString()));
                            out.println(json.toString());
                        }catch (Exception ex) {
                            LOGGER.log(Level.SEVERE, null, ex);
                            JSONObject json = new JSONObject();
